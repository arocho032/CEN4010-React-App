		...
		cards[i] = (
			<Card key={i}>
				...
				<Card.Meta textAlign='center'>
					<span>{events[i].time.date}, {events[i].time.time}</span>
				</Card.Meta>
				<Card.Content>
					<span>{events[i].desc}</span><br/>
					<span><b>Location:</b> {events[i].loc.name}</span>
				</Card.Content>
			</Card>
		)
	}
...
export default compose(withConnect, memo)(EventView)
