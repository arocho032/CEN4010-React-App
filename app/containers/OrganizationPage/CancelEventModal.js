...
function CancelEventModal(props) {
  const userOptions = props.events.map((event, index) => ({
  ...
  return (
    <Modal
      trigger={
        <Dropdown.Item onClick={props.onOpen}>Cancel Event</Dropdown.Item>
      }
...
export function mapDispatchToProps(dispatch) {
  return {
    ...
    onHandleChange: (e, { name, value, checked }) =>
      dispatch(onInputChange(name, 'tempCancelEvent', value, checked)),
  };
}
